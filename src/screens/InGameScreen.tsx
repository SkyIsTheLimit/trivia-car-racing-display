...
export function Loader() {
  return (
    <div className='relative z-10 flex flex-col items-center gap-8'>
      <h1 className='text-4xl'>Next question coming up...</h1>
      <svg
        aria-hidden='true'
        className='w-24 h-24 mr-2 text-gray-200 animate-spin fill-fuchsia-600'
        viewBox='0 0 100 101'
        fill='none'
...
export function InGameScreen({ game }: { game: GameState }) {
  return (
    <Background>
      ...
      {game.currentRound?.status !== 'pre-question' && game.currentRound && (
        <Question
          ...
          displayCorrectAnswer={game.currentRound.status === 'post-question'}
        />
      )}
      ...
      {game.currentRound && !game.currentRound.p1Answer && (
        <PlayerCharacter
          name='bulbasaur'
          className='left-8 bottom-5 -rotate-3'
        />
      )}

      {game.currentRound && !game.currentRound.p2Answer && (
        <PlayerCharacter name='pikachu' className='right-8 bottom-6 rotate-6' />
      )}
    </Background>
  );
}
